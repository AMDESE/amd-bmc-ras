                                             ...
                                             std::string errorType)
{
    ...
    {
        data->header.recordLength = sizeof(CperRecord);
    }
    ...
    for (int i = 0; i < sectionCount; i++)
    {
        ...
        if (errorType == FATAL_ERR)
        {
            ...
            for (int i = 0; i < numOfCpu; i++)
            {
                ...
                data->sectionDescriptor[INDEX_0].fruText[INDEX_1] = '0' + i;
            }
        }
    ...
    int64_t* apmlRetryCount, uint16_t& debugLogIdOffset)
{
    ...
    while (ret != OOB_SUCCESS)
    {
        ...
        {
            // lg2::error("Socket {SOCKET}: Failed to get valid debug log for
            // Dbg Log ID {BLK_ID} . Error =
            // {RET}","SOCKET",info,"BLK_ID",blk_id,"RET",ret);
    ...
    int64_t* apmlRetryCount)
{
    ...
    for (int i = 0; i < sectionCount; i++)
    {
        fatalPtr->errorRecord[i].contextInfo.microcodeVersion = uCode[i];
