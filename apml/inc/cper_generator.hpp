...
class CperGenerator
{
  ...
  public:
     ...
     * @param[in] errCount   The preserved error count number for numbering cper files.
     */
    CperGenerator(uint8_t numOfCpu, uint8_t progId,uint32_t familyId, int errCount) : numOfCpu(numOfCpu),
                               progId(progId) , familyId(familyId) , errCount(errCount) {}
     ...
     */
    void dumpErrorDescriptorSection(const std::shared_ptr<T>&,
                                   uint16_t , std::string ,
                                   uint32_t* );
     ...
     */
    void dumpProcessorErrorSection(const std::shared_ptr<CperRecord>&,uint8_t, uint16_t , CpuId*);
     ...
     *  @param[in] SectionCount - The number of error descriptor sections.
     */
    void cperFileWrite(const std::shared_ptr<T>&, std::string);
     ...
     */
    void dumpContextInfo(const std::shared_ptr<CperRecord>& ,uint16_t, uint16_t, uint8_t ,uint16_t,
                                    std::vector<uint8_t>, uint64_t* , uint32_t*,int64_t*);

    /** @brief Function to dump the last transaction address during a fatal error
     *  @param[in] data - Shared pointer to the CPER record object
     *  @param[in] socNum - socket number.
     */
    void getLastTransAddr(const std::shared_ptr<CperRecord>& , uint8_t );

    /** @brief Function to harvest number of valid debug log instances during a Syncflood.
     *  @param[in] data - Shared pointer to the CPER record object
     *  @param[in] socNum - socket number.
     *  @param[in]  - A vector of uint8_t representing block IDs
     *  @param[in]  - PPIn The processor pin read from apml commands
     ...
     */
    void harvestDebugLogDump(const std::shared_ptr<CperRecord>& , uint8_t , uint8_t ,int64_t* ,uint16_t& );

    /** @brief Function returns cper file name for the provided error count number
