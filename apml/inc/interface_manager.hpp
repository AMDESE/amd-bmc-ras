...
class InterfaceManager : public RasConfiguration
{
  ...
  private:
     ...
     *
     * This function retrieves the version of microcode currently loaded on the CPU.
     */
    void getMicrocodeRev();

    /**
     * @brief Retrieves the PPIN (Platform Processor Identification Number) fuse.
     *
     * This function obtains the PPIN fuse value associated with the hardware platform.
     */
    void getPpinFuse();
     ...
     *
     * This function generates a configuration file with default settings and parameters.
     * The configuration file can ve modified by the user in runtime.
     */
     ...
     */
    template <typename T>
        T getProperty(sdbusplus::bus::bus&, const char*, const char*,
     ...
     * @param[in,out] gpioEventDescriptor - A stream descriptor associated with the GPIO line's events.
     */
     ...
     */
    template <typename T>
    void harvestMcaDataBanks(uint8_t , uint16_t ,
                                   uint16_t , CperGenerator<T>& );
     ...
     *
     * This function triggers a cold reset, which involves a complete system restart.
     * It performs a full power cycle, resetting all components and clearing any
     * volatile state.
     */
  ...
  public:
     ...
     */
     InterfaceManager(sdbusplus::asio::object_server& objectServer,
                  ...
                  apmlInitialized(false) , rcd(nullptr),
                  p0_apmlAlertEvent(io), p1_apmlAlertEvent(io),
                  p0AlertProcessed(false), p1AlertProcessed(false)
    {
    }
     ...
     *
     * This function collects relevant dump data associated with the given error type.
     *
     ...
     *
     * This function executes recovery actions based on the current user configuration.
     * It may perfomr wam reset or cold reset or no reset.
     *
