...
void Manager::updateConfigToDbus() {
  if (!fs::exists(CONFIG_FILE)) { // Check if the config file exists
  ...
  for (const auto &item : data["Configuration"]) {
    ...
    if (item.is_object() && item.size() == 1) {
      ...
      if (value.index() == 0) {
        attributeType = sdbusplus::common::com::amd::ras::Configuration::
        ...
        attributeType = sdbusplus::common::com::amd::ras::Configuration::
            AttributeType::ArrayOfStrings;
      } else if (value.index() == 4) {
        attributeType = sdbusplus::common::com::amd::ras::Configuration::
            AttributeType::KeyValueMap;
      }
