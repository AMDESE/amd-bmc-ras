...
template <typename T>
void dumpErrorDescriptorSection(
    const std::shared_ptr<T>& data, uint16_t sectionCount,
    const std::string& errorType, uint32_t* severity, uint8_t progId,
    uint32_t familyId)
{
                    ...
                    int& errCount)
{
    ...
    if ((errorType == runtimeMcaErr) || (errorType == runtimeDramErr))
    {
        ...
        {
            fwrite(&procPtr->Header, sizeof(EFI_COMMON_ERROR_RECORD_HEADER), 1,
                   file);
            ...
            fwrite(procPtr->McaErrorInfo,
                   sizeof(RUNTIME_ERROR_INFO) * sectionCount, 1, file);
