...
std::string findCperFilename(int number)
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator(kRasDir.data()))
    {
...
{
    sd_journal_print(LOG_INFO, "CreateDbusInterface \n");
    std::future<void> fut;
    ...
    if (std::filesystem::exists(std::filesystem::path(kRasDir.data()))) {
        ...
        for (const auto& p : std::filesystem::directory_iterator(
            ...
            if (!fin.read(reinterpret_cast<char*>(&timestamp), sizeof(timestamp))) {
