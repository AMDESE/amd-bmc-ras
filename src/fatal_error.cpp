...
void dump_processor_error_section(uint8_t info)
{
    ...
    rcd->ErrorRecord[INDEX_0].ProcError.ValidBits =
        CPU_ID_VALID | LOCAL_APIC_ID_VALID;
    rcd->ErrorRecord[INDEX_0].ProcError.CpuId[INDEX_0] = eax[INDEX_0];
    rcd->ErrorRecord[INDEX_0].ProcError.CpuId[INDEX_2] = ebx[INDEX_0];
    rcd->ErrorRecord[INDEX_0].ProcError.CpuId[INDEX_4] = ecx[INDEX_0];
    rcd->ErrorRecord[INDEX_0].ProcError.CpuId[INDEX_6] = edx[INDEX_0];
    rcd->ErrorRecord[INDEX_0].ProcError.CPUAPICId =
        ((ebx[INDEX_0] >> SHIFT_24) & INT_255);

    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        rcd->ErrorRecord[INDEX_1].ProcError.ValidBits =
            CPU_ID_VALID | LOCAL_APIC_ID_VALID;
        rcd->ErrorRecord[INDEX_1].ProcError.CpuId[INDEX_0] = eax[INDEX_1];
        rcd->ErrorRecord[INDEX_1].ProcError.CpuId[INDEX_2] = ebx[INDEX_1];
        rcd->ErrorRecord[INDEX_1].ProcError.CpuId[INDEX_4] = ecx[INDEX_1];
        rcd->ErrorRecord[INDEX_1].ProcError.CpuId[INDEX_6] = edx[INDEX_1];
        rcd->ErrorRecord[INDEX_1].ProcError.CPUAPICId =
            ((ebx[INDEX_1] >> SHIFT_24) & INT_255);

        if (num_of_proc == FOUR_SOCKET)
        {
            rcd->ErrorRecord[INDEX_2].ProcError.ValidBits =
                CPU_ID_VALID | LOCAL_APIC_ID_VALID;
            rcd->ErrorRecord[INDEX_2].ProcError.CpuId[INDEX_0] = eax[INDEX_2];
            rcd->ErrorRecord[INDEX_2].ProcError.CpuId[INDEX_2] = ebx[INDEX_2];
            rcd->ErrorRecord[INDEX_2].ProcError.CpuId[INDEX_4] = ecx[INDEX_2];
            rcd->ErrorRecord[INDEX_2].ProcError.CpuId[INDEX_6] = edx[INDEX_2];
            rcd->ErrorRecord[INDEX_2].ProcError.CPUAPICId =
                ((ebx[INDEX_2] >> SHIFT_24) & INT_255);

            rcd->ErrorRecord[INDEX_3].ProcError.ValidBits =
                CPU_ID_VALID | LOCAL_APIC_ID_VALID;
            rcd->ErrorRecord[INDEX_3].ProcError.CpuId[INDEX_0] = eax[INDEX_3];
            rcd->ErrorRecord[INDEX_3].ProcError.CpuId[INDEX_2] = ebx[INDEX_3];
            rcd->ErrorRecord[INDEX_3].ProcError.CpuId[INDEX_4] = ecx[INDEX_3];
            rcd->ErrorRecord[INDEX_3].ProcError.CpuId[INDEX_6] = edx[INDEX_3];
            rcd->ErrorRecord[INDEX_3].ProcError.CPUAPICId =
                ((ebx[INDEX_3] >> SHIFT_24) & INT_255);
        }
    }

    rcd->ErrorRecord[info].ProcError.ValidBits |= PROC_CONTEXT_STRUCT_VALID;
}
...
void dump_context_info(uint16_t numbanks, uint16_t bytespermca, uint8_t info)
{
    ...
    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        ...
        if (num_of_proc == FOUR_SOCKET)
        {
            ...
            for (blk_id = 0; blk_id < BlockId.size(); blk_id++)
            {
                harvestDebugLogDump(p2_info, BlockId[blk_id]);
            }
            ...
            for (blk_id = 0; blk_id < BlockId.size(); blk_id++)
            {
                harvestDebugLogDump(p3_info, BlockId[blk_id]);
            }
        }
    }

    rcd->ErrorRecord[info].ContextInfo.RegisterContextType = CTX_OOB_CRASH;
    rcd->ErrorRecord[info].ContextInfo.RegisterArraySize =
        numbanks * bytespermca;

    rcd->ErrorRecord[INDEX_0].ContextInfo.MicrocodeVersion = p0_ucode;
    rcd->ErrorRecord[INDEX_0].ContextInfo.Ppin = p0_ppin;

    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        rcd->ErrorRecord[INDEX_1].ContextInfo.MicrocodeVersion = p1_ucode;
        rcd->ErrorRecord[INDEX_1].ContextInfo.Ppin = p1_ppin;

        if (num_of_proc == FOUR_SOCKET)
        {
            rcd->ErrorRecord[INDEX_2].ContextInfo.MicrocodeVersion = p2_ucode;
            rcd->ErrorRecord[INDEX_2].ContextInfo.Ppin = p2_ppin;

            rcd->ErrorRecord[INDEX_3].ContextInfo.MicrocodeVersion = p3_ucode;
            rcd->ErrorRecord[INDEX_3].ContextInfo.Ppin = p3_ppin;
        }
    }
}
                                   ...
                                   uint16_t bytespermca)
{
    ...
    if (num_of_proc == FOUR_SOCKET)
    {
        SectionCount = INDEX_4;
    }
    else
    {
        SectionCount = INDEX_2;
    }

    dump_cper_header_section(rcd, SectionCount, CPER_SEV_FATAL, FATAL_ERR);

    rcd->SectionDescriptor = new error_section_descriptor[SectionCount];
    SectionDesSize = sizeof(error_section_descriptor) * SectionCount;
    memset(rcd->SectionDescriptor, 0, SectionDesSize);

    rcd->ErrorRecord = new ERROR_RECORD[SectionCount];
    SectionDesSize = sizeof(ERROR_RECORD) * SectionCount;
    memset(rcd->ErrorRecord, 0, SectionDesSize);

    dump_error_descriptor_section(rcd, SectionCount, FATAL_ERR, &Severity);
    ...
    while (n < numbanks)
    {
        ...
        {
            rcd->ErrorRecord[info].ProcError.SignatureID[INDEX_0] = mca_synd_lo;
            rcd->ErrorRecord[info].ProcError.SignatureID[INDEX_1] = mca_synd_hi;
            rcd->ErrorRecord[info].ProcError.SignatureID[INDEX_2] = mca_ipid_lo;
            rcd->ErrorRecord[info].ProcError.SignatureID[INDEX_3] = mca_ipid_hi;
            rcd->ErrorRecord[info].ProcError.SignatureID[INDEX_4] =
                mca_status_lo;
            rcd->ErrorRecord[info].ProcError.SignatureID[INDEX_5] =
                mca_status_hi;
        ...
        if (buf & INT_15)
        {
            ...
            else if (num_of_proc == FOUR_SOCKET)
            {
                if ((P0_AlertProcessed == true) &&
                    (P1_AlertProcessed == true) &&
                    (P2_AlertProcessed == true) && (P3_AlertProcessed == true))
                {
                    ResetReady = true;
                }
            }
            ...
            if (ResetReady == true)
            {
                ...
                if (ControlFabricError == false)
                {
                    ...
                    {
                        write_to_cper_file(rcd, FATAL_ERR, INDEX_4);
                    }
                }
...
void P2AlertEventHandler()
{
    ...
    {
        sd_journal_print(LOG_DEBUG, "Rising Edge: P2 APML Alert cancelled\n");
    }
    P2_apmlAlertEvent.async_wait(
        ...
        [](const boost::system::error_code ec) {
            if (ec)
            {
                sd_journal_print(LOG_ERR, "P2 APML alert handler error: %s\n",
                                 ec.message().c_str());
                return;
            }
            P2AlertEventHandler();
...
void P3AlertEventHandler()
{
    ...
    {
        sd_journal_print(LOG_DEBUG, "Rising Edge: P3 APML Alert cancelled\n");
    }
    P3_apmlAlertEvent.async_wait(
        ...
        [](const boost::system::error_code ec) {
            if (ec)
            {
                sd_journal_print(LOG_ERR, "P3 APML alert handler error: %s\n",
                                 ec.message().c_str());
                return;
            }
            P3AlertEventHandler();
