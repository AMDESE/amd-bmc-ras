        ...
        [this](sdbusplus::message::message& message) {
            ...
            if (*currentTimerEnable == false)
            {
                ...
                std::string CurrentTimerUse =
                    ras::util::getProperty<std::string>(
                                  ...
                                  uint16_t bytespermca)
{
    ...
    ras::cper::util::dumpHeaderSection(rcd, sectionCount, errorSeverity,
                                       fatalErr, boardId, recordId);
    ras::cper::util::dumpErrorDescriptorSection(
        rcd, sectionCount, fatalErr, &errorSeverity, progId, familyId);
                                   ...
                                   struct ras_rt_valid_err_inst p1Inst)
{
    ...
    if (errorPollingType == MCA_ERR)
    {
        ...
        {
            dumpProcErrorSection(mcaPtr, 0, p0Inst, MCA_ERR, sectionStart,
                                 severity, checkInfo);

            ras::cper::util::dumpProcErrorInfoSection(
        ...
        if (p1Inst.number_of_inst != 0)
        {
            ...
            dumpProcErrorSection(mcaPtr, 1, p1Inst, MCA_ERR, sectionStart,
                                 severity, checkInfo);
            ras::cper::util::dumpProcErrorInfoSection(
        ...
        ras::util::calculateErrorSeverity(severity, sectionCount,
                                          &highestSeverity, runtimeMcaErr);

        ras::cper::util::dumpHeaderSection(mcaPtr, sectionCount,
                                           highestSeverity, runtimeMcaErr,
                                           boardId, recordId);
        ...
        exportCrashdumpToDBus(errCount - 1, mcaPtr->Header.TimeStamp,
                              objectServer, systemBus);
    ...
    else if (errorPollingType == DRAM_CECC_ERR)
    {
        ...
        if (p0Inst.number_of_inst != 0)
        {
            dumpProcErrorSection(dramPtr, 0, p0Inst, DRAM_CECC_ERR,
        ...
        ras::util::calculateErrorSeverity(severity, sectionCount,
                                          &highestSeverity, runtimeDramErr);

        ras::cper::util::dumpHeaderSection(dramPtr, sectionCount,
                                           highestSeverity, runtimeDramErr,
                                           boardId, recordId);
        ...
        exportCrashdumpToDBus(errCount - 1, dramPtr->Header.TimeStamp,
                              objectServer, systemBus);
    ...
    else if (errorPollingType == PCIE_ERR)
    {
        ...
        if (p0Inst.number_of_inst != 0)
        {
            dumpProcErrorSection(pciePtr, 0, p0Inst, PCIE_ERR, sectionStart,
                                 severity, checkInfo);
        ...
        if (p1Inst.number_of_inst != 0)
        {
            ...
            dumpProcErrorSection(pciePtr, 0, p1Inst, PCIE_ERR, sectionStart,
                                 severity, checkInfo);
        ...
        ras::cper::util::dumpHeaderSection(pciePtr, sectionCount,
                                           highestSeverity, runtimePcieErr,
                                           boardId, recordId);
        ...
        exportCrashdumpToDBus(errCount - 1, pciePtr->Header.TimeStamp,
                              objectServer, systemBus);
...
void Manager::dumpProcErrorSection(
    const std::shared_ptr<T>& data, uint8_t soc_num,
    ...
    while (n < inst.number_of_inst)
    {
        ...
        for (; offset < inst.number_bytes; offset = offset + 4)
        {
            ...
            if (ret != OOB_SUCCESS)
            {
                ...
                else if (PciePtr)
                {
                    ...
                    PciePtr->PcieErrorData[Section]
                        ...
                        (BAD_DATA >> 8) & 0xFF;
                    PciePtr->PcieErrorData[Section]
                        .AerInfo.PcieAer[DumpIndex * 4 + 3] = BAD_DATA & 0xFF;
                }
                continue;
            }
            if (ProcPtr)
            {
                ...
                if (d_in.offset == 8)
                {
                    mca_status_register = mca_status_register |
                                          ((uint64_t)d_out);
                }
                else if (d_in.offset == 12)
                {
                    mca_status_register = ((uint64_t)d_out << 32) |
                                          mca_status_register;
                }
            }
            else if (PciePtr)
            {
                PciePtr->PcieErrorData[Section]
                    .AerInfo.PcieAer[DumpIndex * 4 + 0] = (d_out >> 24) & 0xFF;
                PciePtr->PcieErrorData[Section]
                    .AerInfo.PcieAer[DumpIndex * 4 + 1] = (d_out >> 16) & 0xFF;
                PciePtr->PcieErrorData[Section]
                    .AerInfo.PcieAer[DumpIndex * 4 + 2] = (d_out >> 8) & 0xFF;
                PciePtr->PcieErrorData[Section]
                    .AerInfo.PcieAer[DumpIndex * 4 + 3] = d_out & 0xFF;
