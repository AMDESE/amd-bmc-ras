                                  ...
                                  struct ras_df_err_chk errorCheck)
{
    ...
    while (n < errorCheck.df_block_instances)
    {
        for (uint32_t offset = 0; offset < maxOffset32; offset++)
        {
            ...
            if (dfError.input[0] == statusOffsetHi)
            {
                ...
                /*Bit 29 of MCA_STATUS_HI indicates if the error is
                    uncorrectable*/
                if (mcaStatusHi & (1 << 29))
                {
    ...
    // Increment per-core noncorrectable error counts for cores that
    // had MCA errors, based on IPID decode.
    if (!errorCores.empty())
    {
    ...
    {
        // No per-core bank identified; increment socket-level non-correctable
        // Other error count
...
bool Manager::decodeInterrupt(uint8_t socNum, uint32_t src)
{
    ...
    if (resetReady == true)
    {
                                                         ...
                                                         rcd) == true))
        {
            ...
            std::ifstream inputFile("/home/root/bmcweb_persistent_data.json");
                        ...
                        for (const auto& subscription : subscriptionsArray)
                        {
                            ...
                            if (messageIds.is_array())
                            {
                                ...
                                if (messageIdFound)
                                {
                                    ...
                                    struct ras_override_delay dataIn = {0, 0,
                                                                        0};
                                    bool ackResp;
        ...
        if (recoveryAction == true)
        {
            ...
            amd::ras::util::rasRecoveryAction(node, (uint8_t)src,
