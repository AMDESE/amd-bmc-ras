                  ...
                  std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    ...
    if (amd::ras::dbus::util::checkObjPath(dbusPath) == true)
    {
                        ...
                        uint8_t cpuCount)
{
    for (size_t i = 0; i < cpuCount; i++)
    {
        ...
        fatalPtr->ErrorRecord[i].ProcError.CpuIdInfo[doubleBit] = cpuId[i].ebx;
        fatalPtr->ErrorRecord[i].ProcError.CpuIdInfo[quadBit] = cpuId[i].ecx;
                 ...
                 const std::unique_ptr<uint32_t[]>& uCode, uint8_t cpuCount)
{
    for (size_t i = 0; i < cpuCount; i++)
    {
        ...
        if (i == socNum)
        {
            fatalPtr->ErrorRecord[i].RegisterContextType =
                           ...
                           const std::shared_ptr<FatalCperRecord>& rcd)
{
    ...
    for (socNum = 0; socNum < socket2; socNum++)
    {
        ...
        {
            bool equal = amd::ras::hex::util::compareBitwiseAnd(tempVar[socNum],
                                                                pair.second);
