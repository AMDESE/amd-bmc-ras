                                   ...
                                   uint32_t* Severity)
{
    for (int i = 0; i < SectionCount; i++)
    {
        ...
        if (ErrorType == FATAL_ERR)
        {
            ...
            data->SectionDescriptor[i].FRUText[INDEX_1] =
                              ...
                              std::string ErrorType)
{
    ...
    else if (ErrorType == FATAL_ERR)
    {
        data->Header.RecordLength =
            sizeof(common_error_record_header) +
            sizeof(ERROR_SECTION_DESCRIPTOR) * SectionCount +
            sizeof(ERROR_RECORD) * SectionCount;
    }
    ...
    else if (ErrorType == FATAL_ERR)
    {
        if ((FatalPtr) && (file != NULL))
        {
            ...
            if (FatalPtr != nullptr)
            {
                ...
                fwrite(&FatalPtr->Header, sizeof(common_error_record_header), 1,
                       file);

                if (FatalPtr->SectionDescriptor != nullptr)
                {
                    fwrite(FatalPtr->SectionDescriptor,
                           sizeof(ERROR_SECTION_DESCRIPTOR) * SectionCount, 1,
                           file);
                }

                if (FatalPtr->ErrorRecord != nullptr)
                {
                    fwrite(FatalPtr->ErrorRecord,
                           sizeof(ERROR_RECORD) * SectionCount, 1, file);
                }
