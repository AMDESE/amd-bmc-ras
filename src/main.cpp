...
std::string P2_InventoryPath =
    "/xyz/openbmc_project/inventory/system/processor/P2";
std::string P3_InventoryPath =
    "/xyz/openbmc_project/inventory/system/processor/P3";
...
gpiod::line P2_apmlAlertLine;
gpiod::line P3_apmlAlertLine;
...
boost::asio::posix::stream_descriptor P2_apmlAlertEvent(io);
boost::asio::posix::stream_descriptor P3_apmlAlertEvent(io);
...
uint32_t eax[INDEX_4], ebx[INDEX_4], ecx[INDEX_4], edx[INDEX_4];
uint32_t p0_ucode = 0, p1_ucode = 0, p2_ucode = 0, p3_ucode = 0;
uint64_t p0_ppin = 0, p1_ppin = 0, p2_ppin = 0, p3_ppin = 0;
...
void getCpuID()
{
    ...
    ret = esmi_oob_cpuid(p0_info, core_id, &eax[INDEX_0], &ebx[INDEX_0],
                         &ecx[INDEX_0], &edx[INDEX_0]);
    ...
    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        ...
        ret = esmi_oob_cpuid(p1_info, core_id, &eax[INDEX_1], &ebx[INDEX_1],
                             &ecx[INDEX_1], &edx[INDEX_1]);
        ...
        if (num_of_proc == FOUR_SOCKET)
        {
            ...
            ret = esmi_oob_cpuid(p2_info, core_id, &eax[INDEX_2], &ebx[INDEX_2],
                                 &ecx[INDEX_2], &edx[INDEX_2]);

            if (ret)
            {
                sd_journal_print(LOG_ERR,
                                 "Failed to get the CPUID for socket 2\n");
            }
            ...
            ret = esmi_oob_cpuid(p3_info, core_id, &eax[INDEX_3], &ebx[INDEX_3],
                                 &ecx[INDEX_3], &edx[INDEX_3]);

            if (ret)
            {
                sd_journal_print(LOG_ERR,
                                 "Failed to get the CPUID for socket 3\n");
            }
        }
    }
}
...
void getMicrocodeRev()
{
    ...
    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        ...
        {
            std::string MicroCode = getProperty<std::string>(
                bus, InventoryService.c_str(), P2_InventoryPath.c_str(),
                CpuInventoryInterface, "Microcode");
            ...
            if (MicroCode.empty())
            {
                sd_journal_print(
                    LOG_ERR,
                    "Failed to read ucode revision for Processor P2\n");
            }
            else
            {
                p2_ucode = std::stoul(MicroCode, nullptr, BASE_16);
            }

            MicroCode = getProperty<std::string>(
                bus, InventoryService.c_str(), P3_InventoryPath.c_str(),
                CpuInventoryInterface, "Microcode");
            ...
            {
                p3_ucode = std::stoul(MicroCode, nullptr, BASE_16);
            }
        }
    }
}
...
void getPpinFuse()
{
    ...
    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        ...
        if (num_of_proc == FOUR_SOCKET)
        {
            ...
            {
                sd_journal_print(LOG_ERR,
                                 "Failed to read Ppin for Processor P1\n");
            }
            else
            {
                p2_ppin = std::stoull(Ppin, nullptr, BASE_16);
            }

            Ppin = getProperty<std::string>(bus, InventoryService.c_str(),
                                            P3_InventoryPath.c_str(),
                                            CpuInventoryInterface, "PPIN");
            if (Ppin.empty())
            {
                sd_journal_print(LOG_ERR,
            ...
            {
                p3_ppin = std::stoull(Ppin, nullptr, BASE_16);
            }
        }
    }
}
...
int main()
{
    ...
    if ((num_of_proc == TWO_SOCKET) || (num_of_proc == FOUR_SOCKET))
    {
        ...
        if (num_of_proc == FOUR_SOCKET)
        {
            requestGPIOEvents("P2_I3C_APML_ALERT_L", P2AlertEventHandler,
                              P2_apmlAlertLine, P2_apmlAlertEvent);

            requestGPIOEvents("P3_I3C_APML_ALERT_L", P3AlertEventHandler,
                              P3_apmlAlertLine, P3_apmlAlertEvent);
        }
    }
